...
import pygame as pg
from data import config as cng
import random
import math
...
class Obstacle(Object):
    ...
    def __init__(self, image, pos_x, pos_y):
        ...
        self.rect = pg.rect.Rect(self.position.x, self.position.y, self.width, self.height)
        self.speed_x = random.uniform(0.3, 1) #få tilfeldig tall mellom 0.3 og 1

    def got_shot(self, other):
        """Increases score for shooter, and decreases health if got shot"""
        self.health += cng.HITPOINT
        other.score += cng.SCOREPOINT
        #Health increases to 75% if under
        if other.health < 375:
            other.health = 375
...
class Manager():
    ...
    def load_images(self):
        ...
        self.arena1_pic = pg.transform.scale(self.arena1_pic,(self.arena1_pic.get_width()/15, self.arena1_pic.get_height()/15)) 
...
# ============================================================= 
    ...
    def sprites_init(self):
        ...
        for _ in range(cng.OBSTACLES):
            self.obstacle_group.add(Obstacle(self.obstacle_img, random.randint(cng.BORDER, cng.SCREEN_X-cng.BORDER), random.randint(cng.BORDER, cng.SCREEN_Y//2)))
...
# ============================================================= 
    ...
    def update_sprites(self):
        ...
        # Check if obst is shot
        for obst in self.obstacle_group:
            collide_list1 = pg.sprite.spritecollide(obst, self.p1_bullet_group, True)
            if len(collide_list1) > 0:
                obst.got_shot(self.player1)
                if len(self.obstacle_group)<cng.OBSTACLES:
                    self.obstacle_group.add(Obstacle(self.obstacle_img, 0 - self.obstacle_img.get_width(), random.randint(cng.BORDER, cng.SCREEN_Y//2)))
                
            collide_list2 = pg.sprite.spritecollide(obst, self.p2_bullet_group, True)
            if len(collide_list2) > 0:
                obst.got_shot(self.player2)
                if len(self.obstacle_group)<cng.OBSTACLES:
                    self.obstacle_group.add(Obstacle(self.obstacle_img, 0 - self.obstacle_img.get_width(), random.randint(cng.BORDER, cng.SCREEN_Y//2)))
...
# ============================================================= 
    def show_credit(self):
        ...
        creators = font1.render('DEVELOPERS & SUPPORT', True, (255, 255, 255))
        game_devs = font1.render('Game developers: Tora K. Homme & Dulmini S. Gamage', True, (255, 255, 255))
